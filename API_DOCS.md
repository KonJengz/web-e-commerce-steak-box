...
- ถ้าเรียกจาก browser และต้องการให้ cookie ทำงาน ต้องใช้ `credentials: "include"` ใน `fetch` หรือ `withCredentials: true` ใน Axios โดยเฉพาะ `verify-email`, `login`, `google/login`, `github/login`, `oauth/exchange`, `refresh`, `logout`
...
| `POST /api/auth/refresh`             | `401`  | `No refresh token cookie` / `Invalid refresh token` / `Refresh token has expired`                  | ล้าง state auth ฝั่ง frontend แล้วพาไปหน้า login                |
...
```json
{
  "ticket": "9fd4df75b8f10f7e4b02f5e5f6f8b6f79f0f52d0ee2a8d36b6ea5d9f7bde6c15"
}
```
